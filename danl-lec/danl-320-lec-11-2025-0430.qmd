...
- We look at two classes of unsupervised methods:
  ...
  - **Principal component analysis (PCA)** turns many variables into a few new ones, called **principal components**, that still capture most of your data.
      - PCA is a popular **dimensionality reduction** method!
  

## Clustering
- In cluster analysis, the goal is to *group the observations* in your data into *clusters* such that **every observation in a cluster is more similar to other observations in the same cluster than observations in other clusters**.
...
- You can scale numeric data using the function
`StandardScaler()`.

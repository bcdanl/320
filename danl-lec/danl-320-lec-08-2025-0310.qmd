...
- Suppose we want to know how much the flight’s origin/destination, weather, and air carrier affect  *the probability that a flight will be delayed*. 
- Suppose we also want to predict *whether or not a flight will be delayed*, based on facts like the flight’s origin/destination, weather, and air carrier. 
...
&\qquad\quad\, + \texttt{b$_{\texttt{air\_carrier}}$*air_carrier[i] )}.
\end{align}
$$
...
- The logistic function $G(z_i)$ maps the linear combination of predictors to the probability that the outcome $y_{i}$ is $\texttt{TRUE}$.

- $z_{i} = b_{0} + b_{1}x_{1, i} + b_{2}x_{2, i}$
...
- **Task 1.** Identify the effects of predictors on the probability of $\texttt{atRisk == TRUE}$.

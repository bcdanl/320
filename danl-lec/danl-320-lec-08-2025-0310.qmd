...
- A null model (one that predicts the rare event never happens) would be 99% accurate, as it correctly classifies 99% of cases as “not at risk.”
- A useful but imperfect model that flags 5% of cases as at-risk and captures all true at-risk cases may have lower overall accuracy than the null model.
- However, this does not mean the null model is better—it just highlights that accuracy is a poor metric for imbalanced data.
...
- This is not a paradox—it simply reflects that accuracy alone does not capture the effectiveness of a model when class distributions are skewed.

...
```{.python}
path = 'https://bcdanl.github.io/data/nba.csv'
nba = spark.read.csv(path, 
                    inferSchema=True,
                    header=True)
nba.show()
```
...
- What should we do then?
  ...
  - `nba = spark.createDataFrame(df_pd)`
...
nba.select("Name").show(5)

# Multiple columns -> pass a list-like of column names
nba.select("Name", "Team", "Salary").show(5)
```
...
nba_count = nba.count()
print("Number of rows:", nba_count)

# Count distinct values in one column
from pyspark.sql.functions import countDistinct
num_teams = nba.select(countDistinct("Team")).collect()[0][0]
print("Number of distinct teams:", num_teams)

# GroupBy a column and count occurrences
nba.groupBy("Team").count().show(5)
```
...
nba.orderBy("Name").show(5)

# Sort by descending
from pyspark.sql.functions import desc
nba.orderBy(desc("Salary")).show(5)

# Sort by multiple columns
nba.orderBy(["Team", desc("Salary")]).show(5)
```
...
```{.python}
# nsmallest example:
nba.orderBy("Salary").limit(5).show()

# nlargest example:
nba.orderBy(desc("Salary")).limit(5).show()
```
...
nba.filter("Team == 'Lakers'").show()

# or using a column expression
from pyspark.sql.functions import col
nba.filter(col("Team") == "Lakers").show()
```
...
nba = nba.withColumn("Salary_k", col("Salary") / 1000)
```

#### Removing Columns with `drop()`

```{.python}
nba = nba.drop("Salary_k")  # remove a single column
nba = nba.drop("Salary_2x", "Salary_3x")  # remove multiple columns
```
...
```{.python}
nba = nba.withColumnRenamed("Birthday", "DateOfBirth")
```
...
```{.python}
nba = nba.select("Name", "Team", "Position", "Salary", "DateOfBirth")
```
...
salary_mean = nba.select(F.avg("Salary").alias("mean_salary")).collect()[0]["mean_salary"]
...
nba = nba.withColumn("Salary_int", col("Salary").cast("int"))

# Convert to date or timestamp
nba = nba.withColumn("DateOfBirth_ts", col("DateOfBirth").cast("timestamp"))
```
...
emp = spark.createDataFrame(df_pd)

emp.filter(col("Salary") > 100000).show()
```
...
emp.filter(col("Team").isin("Finance", "Legal", "Sales")).show()
```
...
```{.python}
emp_between = emp.filter(col("Salary").between(90000, 100000))
emp_between.show()
```
...
emp.filter(col("Team").isNull()).count()

# Similarly, you can filter non-null
emp.filter(col("Team").isNotNull()).count()
```
...
emp_drop_all = emp.na.drop(how="all")
```
...
emp_drop_subset = emp.na.drop(subset=["Gender", "Team"])
```
...
emp_fill = emp.na.fill(value = 0, subset = ["Salary"])
```
...
emp_fill_multi = emp.na.fill({"Salary": 0, "Team": "Unknown"})
```
...
emp_no_dups = emp.dropDuplicates()

# Drop duplicates based on subset of columns
emp_no_dups_subset = emp.dropDuplicates(["Team"])
```

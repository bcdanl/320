...
&\,+\, \sum_{j=N+1}^{N+4}\beta_{j\times\text{promo}}\,\text{brand}_{j}\times \text{promo}\times \log(\text{beer\_floz})\\
&\,+\, \epsilon
\end{aligned}
$$

- Set "**BUD LIGHT**" as the reference level for $\text{brand}$. 
- Set "**BUFFALO-ROCHESTER**" as the reference level for $\text{market}$. 
